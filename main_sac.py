...
class RobotArmPickPlaceEnvHER(gym.Env):
    def __init__(self, gui=True, debug_mode=False):
        ...
        self.stagnation_detection = {
            ...
            'hovering_threshold': 5,  # REDUZIERT: Frhere Erkennung für strkeren Lerndruck - reines RL-Training
            'descent_motivation_active': False  # Zeigt an ob Absenkungsmotivation aktiv ist (nur für Monitoring)
        }
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.grasp_constraint is not None:
            p.removeConstraint(self.grasp_constraint)
        self.grasp_constraint = None
          # Performance Tracking zurcksetzen (ERWEITERT für Anti-Lokale-Optima)
        self.last_distance = float
        self.last_gripper_distance = float('inf')  # für Verbesserungs-Tracking
        self.last_transport_distance = float('inf')  # für Transport-Verbesserung
    ...
    def _update_grasp_state_v2(self):
        ...
        # LOSLASSEN: Greifer weit geffnet ODER strategisches Loslassen in der Kiste
            ...
            # Prfe ob wir ber der Kiste sind für kontrolliertes Loslassen
    ...
    def step(self, action):
        ...
        try:
            ...
            height_above_cube = end_effector_pos[2] - cube_pos[2]
              #  PRZISES AUTO-GREIF-SYSTEM: Optimale teilweise Schlieung für perfekten Wrfelgriff
            ...
            if auto_grasp_conditions and current_gripper_pos > 0.025:
                ...
                # Ziel: 0.020 (optimale Greifposition für Wrfel) statt voll geschlossen
            ...
            if has_contact and gripper_target < current_gripper_pos:
                ...
                if both_contacts:
                    # Bei beidseitigem Kontakt: optimale Greifposition für Wrfel
                    gripper_target = 0.020  # Perfekte Position für den Wrfel
                elif left_contact or right_contact:
                    # Bei einseitigem Kontakt: etwas weiter offen lassen für bessere Zentrierung
                    gripper_target = 0.022  # Leicht offener für Anpassung
        ...
        truncated = self.episode_step >= self.max_episode_steps  # Zeitlimit erreicht
          # Aktuelle Wrfel position für Debug
        cube_pos, _ = p.getBasePositionAndOrientation(self.cube_id)
        cube_pos = np.array(cube_pos)
    ...
    def _update_grasp_state(self):
        try:
            ...
            # NaN-Check für distance_to_cube
            ...
            # NaN-Check für Greifer-Positionen
            if np.isnan(gripper_pos_left) or np.isnan(gripper_pos_right):
                print(f" NaN in gripper positions: left={gripper_pos_left}, right={gripper_pos_right}")
                return  # Abort dieser Aktualisierung
        ...
        if distance_to_cube < 0.08 and 0.015 < avg_gripper_pos < 0.025:
            if self.grasp_constraint is None:                # Prüfe Kontakt zwischen Greifer-Fingern und Würfel
                ...
                if both_contacts or (left_contact or right_contact) and xy_distance < 0.04:
                    ...
                    # End-Effector-Orientierung für Berechnung der relativen Position
                    ...
                    # Feinabstimmung der Positionierung für bessere Zentrierung
                    if both_contacts:
                    ...
                    # Hhenanpassung für optimale Position
                    local_pos[2] -= 0.01  # Leicht nach unten versetzt für stabilen Griff
                    
                    # VERBESSERTE CONSTRAINT-ERSTELLUNG für przise Wrfelplatzierung
                    self.grasp_constraint = p.createConstraint(
                        parentBodyUniqueId=self.robot_id,
                        parentLinkIndex=self.end_effector_link_index,
                        childBodyUniqueId=self.cube_id,
                        ...
                        jointType=p.JOINT_FIXED,
                        jointAxis=[0, 0, 0],
                        parentFramePosition=local_pos,  # Verwende lokale Position für przise Platzierung
                        childFramePosition=[0, 0, 0]    # Wrfelmitte
                    )
                    
                    # Nach erfolgreichem Griff: optimale Greifposition einstellen und halten
                    optimal_gripper_pos = 0.020  # Optimale Position für Wrfel
                    for joint_idx in [9, 10]:
                        p.setJointMotorControl2(
                            self.robot_id, joint_idx, p.POSITION_CONTROL,
                            targetPosition=optimal_gripper_pos, 
                            force=60,  # Erhhte Kraft für stabilen Halt
                            maxVelocity=0.3
                        )
    ...
    def _compute_single_reward(self, achieved_goal, desired_goal):
        ...
        try:
            if xy_distance < 0.08 and height_above_cube > 0.03:
                # Progressive Belohnung für Absenkung
                if 0.03 < height_above_cube < 0.15:
                    descent_progress = (0.15 - height_above_cube) / 0.12
                    total_reward += descent_progress * 15.0  # Sehr hohe Belohnung für Absenkung
                ...
                if xy_distance < 0.06 and height_above_cube > 0.10:  # Gut positioniert aber zu hoch
                    ...
                    # DEUTLICH VERSTRKTE RL-BESTRAFUNG für Hovering - muss extrem stark sein
                    # Quadratische Straffunktion für schnellere Eskalation bei lngerem Hovering
                ...
                # Z-Position für nchste Vergleiche speichern
                self.last_z_pos = end_effector_pos[2]
            else:
                self.hovering_steps = 0
                if hasattr(self, 'last_z_pos'):
        ...
        # Mächtige Belohnung für erfolgreiches Greifen
        if is_grasped:
            total_reward += 35.0  # Erhht von 30.0 für strkere Motivation
            phase_info += "GRASPED! "
            phase_info += "GRASPED! "
        ...
        if is_grasped:
            # Progressive Belohnung für Transport
            ...
            # Verbesserungsbonus für Transport
        ...
        # 
        # Einfache Strafe für Kollision mit der Kiste (auer beim Ablegen)
        try:
            if not is_grasped:  # Nur wenn kein Wrfel gegriffen ist
                ...
                if len(tray_contacts) > 0:
                    total_reward -= 3.0  # Strafe für Kollision
    ...
    def _apply_adaptive_exploration(self, action):
        ...
        try:
            ...
            # Reduziert von 12cm auf 6cm für bessere Feinpositionierung
        ...
        if self.stagnation_detection['hovering_steps'] > hovering_threshold:
            if self.debug_mode:
                print(f" REINES RL-TRAINING für ANTI-HOVERING! {self.stagnation_detection['hovering_steps']} steps hovering")
                
            # Nur Information speichern für Debug/Monitoring - keine Aktionsnderung!
        ...
        if force_exploration:
            ...
            if strategy == 0:  # RANDOM MOVEMENT
                ...
                # Spezielle Strategie für Absenkung wenn nah am Wrfel
                if gripper_to_cube_dist < 0.15:
